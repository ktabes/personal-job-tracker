...
Each role report message has one `Apply` control and one `Hide` control. Each opens a popup modal where you enter role numbers shown in the report, such as `1, 3, 5-7`. `Hide` also asks for `7`, `30`, or `forever`; hidden roles are excluded from future reports until their suppression expires.
