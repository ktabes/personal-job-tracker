...
Each role report message has one `Apply` control and one `Hide` control. Each opens a popup modal where you enter the role number shown in the report. `Hide` also asks for `7`, `30`, or `forever`; hidden roles are excluded from future reports until their suppression expires.
